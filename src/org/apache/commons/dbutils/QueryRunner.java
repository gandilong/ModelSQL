...
public class QueryRunner extends AbstractQueryRunner {
    ...
    public <T> T queryModel(String sql, ResultSetHandler<T> rsh,Class<T> type) throws SQLException {
        Connection conn = this.prepareConnection();
        return this.<T>queryModel(conn, true, sql, rsh, type);
    }
    ...
    private <T> T queryModel(Connection conn, boolean closeConn, String sql, ResultSetHandler<T> rsh, Class<T> type)throws SQLException {
		if (conn == null) {
		    throw new SQLException("Null connection");
		}
		...
		try {
		    stmt = this.prepareStatement(conn, sql);
		    this.fillStatement(stmt, (Object[])null);
		    rs = this.wrap(stmt.executeQuery());
		    result = rsh.handle(rs,type);
		
		} catch (SQLException e) {
		    this.rethrow(e, sql, (Object[])null);
